...
export const resumesKeys = {
  ...
  history: (resumeId: number, refId: number) => [...resumesKeys.all, 'history', resumeId, refId] as const,
  companies: (isRecruter: boolean) => [...resumesKeys.all, 'companies', isRecruter] as const,
  contacts: (refId: number, companyId: number) => [...resumesKeys.all, 'contacts', refId, companyId] as const,
};
