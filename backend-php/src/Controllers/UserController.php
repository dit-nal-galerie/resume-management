...
class UserController
{
  ...
  public function createOrUpdateUser($request, $res): Response
  {
    ...
    if (
      !isset($data['email']) ||
      ($data['isNew'] &&
        (empty($data['loginname']) || empty($data['password']) || empty($data['password2'])))
    ) {
  ...
  public function changeAccessData(Request $request, Response $response): Response
  {
    ...
    try {
      ...
      if (!$row) {
        $response
          ->getBody()
          ->write(json_encode(['error' => 'backend.error.notFound.userNotFound']));
        return $response->withStatus(404)->withHeader('Content-Type', 'application/json');
      }
      ...
      if (!password_verify($oldPassword, $row['password'])) {
        $response->getBody()->write(
          json_encode([
            'error' => 'backend.error.auth.oldPasswordWrong' + ' ' + $oldPassword,
          ]),
        );
      ...
      $stmt2 = $this->db->prepare(
        'SELECT COUNT(*) FROM users WHERE email = :email AND loginId != :loginId',
      );
      $stmt2->execute(['email' => $email, 'loginId' => $loginId]);
      if ($stmt2->fetchColumn() > 0) {
        $response
          ->getBody()
          ->write(json_encode(['message' => 'backend.error.conflict.emailTaken']));
        return $response->withStatus(411)->withHeader('Content-Type', 'application/json');
      }
      ...
      if ($changePassword) {
        if ($newPassword !== $password2) {
          $response
            ->getBody()
            ->write(json_encode(['message' => 'backend.error.validation.passwordMismatch']));
          return $response->withStatus(400)->withHeader('Content-Type', 'application/json');
        }
      ...
      $updAuth = $this->db->prepare(
        'UPDATE authentification SET loginname = :loginname, password = :password WHERE id = :loginid',
      );
      $updAuth->execute([
        'loginname' => $loginname,
      ...
      $response
        ->getBody()
        ->write(json_encode(['error' => 'backend.error.server.credentialChangeError']));
      return $response->withStatus(500)->withHeader('Content-Type', 'application/json');
    }
  }
