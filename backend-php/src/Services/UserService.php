...
class UserService
{
  ...
  public function createOrUpdateUser($data, $res): Response
  {
    if (
      ...
      $res
        ->getBody()
        ->write(json_encode(['message' => 'backend.error.validation.missingRequiredFields']));
  ...
  private function createUser(array $data, Response $res): Response
  {
    ...
    if ($stmt->fetch()) {
      $res
        ->getBody()
        ->write(json_encode(['message' => 'backend.error.validation.userAlreadyExists']));
    ...
    try {
      ...
      $res
        ->getBody()
        ->write(json_encode(['message' => 'backend.error.internal', 'error' => $e->getMessage()]));
  ...
  public function login($data)
  {
    ...
    $stmt = $this->db->prepare(
      'SELECT id, password FROM authentification WHERE loginname = :loginname',
    );
