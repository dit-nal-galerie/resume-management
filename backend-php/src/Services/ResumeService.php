...
class ResumeService
{
    ...
    {
        $st = $this->db->prepare("SELECT userid, loginid FROM users WHERE userid=:id OR loginid=:id LIMIT 1");
        $st->execute(['id' => $id]);
    ...
    private function upsertCompany(?array $company, int $loginid): ?int
    {
        ...
        if ($companyId > 0) {
            ...
            $st->execute(compact('name','city','street','houseNumber','postalCode','isrecruter','companyId'));
            return $companyId;
        }
        ...
        $st->execute([
            ...
            'postalCode'=>$postalCode,'isrecruter'=>$isrecruter,'ref'=>$loginid
        ]);
    ...
    private function upsertContact(?array $contact, ?int $companyId, int $loginid): ?int
    {
        ...
        $contactid = isset($contact['contactid']) ? (int)$contact['contactid']
                   : (isset($contact['contactId']) ? (int)$contact['contactId'] : 0);
        ...
        if ($contactid > 0) {
            ...
            $st = $this->db->prepare($sql);
            $st->execute([
                'anrede'=>$anrede,'title'=>$title,'vorname'=>$vorname,'zusatzname'=>$zusatz,
                'name'=>$name,'email'=>$email,'phone'=>$phone,'mobile'=>$mobile,
                'company'=>$companyId,'ref'=>$loginid,'contactid'=>$contactid
            ]);
            return $contactid;
        }
    ...
    {
        $st = $this->db->prepare("INSERT INTO history (resumeid, stateid, date) VALUES (:rid,:sid,:dt)");
    ...
    public function getResumesWithUsers(Request $request, Response $response): Response
    {
        ...
        if ($loginid === null) {
            $response->getBody()->write(json_encode(['success'=>false,'error'=>'backend.error.auth.unauthorized']));
...
SQL;
    ...
    public function updateOrCreateResume(Request $request, Response $response): Response
    {
        ...
        try {
            ...
            $contactParentCompanyId = $this->upsertContact($contactRecrutingCompany, $parentCompanyId, $loginid);
            ...
            $response->getBody()->write(json_encode([
                'success'=>false,'error'=>'backend.error.server.serverError','details'=>$e->getMessage()
            ]));
    ...
    public function changeResumeStatus(Request $request, Response $response): Response
    {
        ...
        $ins = $this->db->prepare("INSERT INTO history (resumeid, stateid, date) VALUES (:rid,:sid,:dt)");
