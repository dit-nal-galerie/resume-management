...
export const createToken = async (pool: Pool, userId: number): Promise<string> => {
  try {
    ...
    throw new Error('Не удалось создать токен восстановления пароля');
  }
};
...
export const markTokenAsUsed = async (pool: Pool, token: string): Promise<boolean> => {
  try {
    ...
    throw new Error('Fehler beim Aktualisieren des Token-Status');
  }
};
...
export const sendPasswordResetEmail = async (to: string, token: string): Promise<boolean> => {
  ...
  try {
    ...
    if (!EMAIL_CONFIG.auth.user || !EMAIL_CONFIG.auth.password) {
      console.warn('Email-Konfiguration fehlt. E-Mails zum Zurücksetzen des Passworts werden nicht gesendet.');
      return false;
    }
...
if (EMAIL_CONFIG.host === 'smtp.ethereal.email') {
  console.log('Тестовое письмо отправлено. URL для просмотра:', 
 ...
 */
export const requestPasswordReset = async (pool: Pool, req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!loginname || !email) {
      res.status(400).json({
        success: false,
        error: 'Loginname und E-Mail sind erforderlich'
      });
      return;
    }
 ...
 * @param res - HTTP-ответ
 */
export const checkPasswordResetToken = async (pool: Pool, req: Request, res: Response): Promise<void> => {
  try {
...
export const resetPassword = async (pool: Pool, req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (newPassword.length < 2) {
      res.status(400).json({
        success: false,
        error: 'Das Passwort muss mindestens 2 Zeichen lang sein'
      });
      return;
    }
