    ...
    try {
        if (company.companyId === 0) {
            ...
            const [result] = await db.query<ResultSetHeader>(insertQuery, [
                company.name, company.city, company.street, company.houseNumber,
                company.postalCode, company.isRecruter || false, ref // Standardwert für isrecruter falls nicht gesetzt
            ]);
            if (result.insertId) {
            ...
            const [result] = await db.query<ResultSetHeader>(updateQuery, [
                company.name, company.city, company.street, company.houseNumber,
                company.postalCode, company.isRecruter || false, company.companyId
            ]);
            console.log(`✅ Firma aktualisiert mit ID: ${company.companyId}. Betroffene Zeilen: ${result.affectedRows}`);
...
async function handleContact(
    ...
    try {
        if (contactid === 0) {
            ...
            `;
            const [result] = await db.query<ResultSetHeader>(insertQuery, [
                vorname, name, email, finalAnrede, title, zusatzname, phone, mobile, companyId, ref
            ]);
            if (result.insertId) {
            ...
            const [result] = await db.query<ResultSetHeader>(updateQuery, [
                vorname, name, email, finalAnrede, title, zusatzname, phone, mobile, companyId, contactid
            ]);
            console.log(`✅ Kontakt aktualisiert mit ID: ${contactid}. Betroffene Zeilen: ${result.affectedRows}`);
            return contactid;
        } else {
        ...
        console.error(`❌ Fehler bei der Verarbeitung von Kontakt ${contactid} für Firma ${companyId}:`, error.message);
    ...
    try {
        if (isNewResume) {
            ...
            if (lastStateRows.length > 0) {
                ...
                console.log(`ℹ️ Letzter bekannter Status für Resume ${resumeId} ist ${lastStateId}. Neuer Status ist ${newStateId}.`);
                if (lastStateId === newStateId) {
                    needsInsert = false;
                    console.log(`ℹ️ Status hat sich nicht geändert. Kein neuer History-Eintrag für Resume ${resumeId}.`);
                }
            } else {
                 console.log(`ℹ️ Keine vorherige History für Resume ${resumeId} gefunden. Füge ersten Eintrag hinzu.`);
            }

            if (needsInsert) {
                console.log(`📝 Füge History für bestehendes Resume ${resumeId} mit neuem Status ${newStateId} hinzu.`);
                const insertHistoryQuery = `
                    INSERT INTO history (resumeid, date, stateid)
                    VALUES (?, CURDATE(), ?) -- Oder NOW() wenn Spalte DATETIME ist
                `;
...
export const updateOrCreateResume = async (dbPool: Pool, req: Request, res: Response): Promise<void> => {

    let connection: PoolConnection | null = null;
    const resume: Resume = req.body;

    // Grundlegende Eingabevalidierung
    if (!resume || !resume.ref || !resume.position || resume.stateId === undefined || resume.stateId === null) {
        res.status(400).send("Fehlende erforderliche Resume-Daten (ref, position, stateId).");
        return;
    }
    ...
    try {
        ...
        const contactCompanyId = await handleContact(connection, resume.contactCompany, companyId, userRef);
        // Kontakt für den Recruiter (nur wenn parentCompanyId gültig ist)
        const contactParentCompanyId = await handleContact(connection, resume.contactRecrutingCompany, parentCompanyId, userRef);
        ...
        if (isNewResume) {
            ...
            console.log(`✅ Resume ${finalResumeId} aktualisiert. Betroffene Zeilen: ${result.affectedRows}`);
            // Optional: Prüfen ob result.affectedRows > 0, um sicherzustellen, dass etwas geändert wurde.
             if (result.affectedRows === 0) {
                  console.warn(`Keine Zeile für Resume ID ${finalResumeId} und User Ref ${userRef} zum Aktualisieren gefunden.`);
        ...
        res.status(isNewResume ? 201 : 200).json({
            message: `Resume erfolgreich ${isNewResume ? 'erstellt' : 'aktualisiert'}.`,
        ...
        if (!res.headersSent) {
            res.status(500).json({
                 message: "Fehler bei der Verarbeitung des Resumes.",
